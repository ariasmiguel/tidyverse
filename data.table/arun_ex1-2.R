...
# 10. Subset DF1 by DF2 on id, code column. That is, for each row of DF2$id, DF2$code, 
# get valA and valB cols from DF1. Include rows that have no matches as well.
ans10 <- DT1[DT2, on = c("id","code"), c("valA", "valB")]

# 11. Same as (10), but fetch just the first matching row of DF1 for each row of DF2$id, 
# DF2$code. Exclude non-matching rows.
...
# 13. Add a new column val to DF1 with values from DF2$mul where DF2$id, DF2$code matches with DF1’s. 
# Rows that don’t match should have NA.
